...
public class PictureSelectorAdapter extends RecyclerView.Adapter {
    ...
    private void setImgChcek(ImageEntity item, ImageView imgSelector) {
        if (item.isSelected()) {
            imgSelector.setBackgroundResource(R.drawable.common_icon_check_selected);
        } else {
            imgSelector.setBackgroundResource(R.drawable.ps_ic_unchecked);
        }
