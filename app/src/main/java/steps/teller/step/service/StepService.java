package steps.teller.step.service;
...
import steps.teller.step.accelerometer.StepCount;
import steps.teller.step.accelerometer.StepValuePassListener;
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                stepData = list.get(i);
                if (TextUtils.isEmpty(stepData.getSecond())) {
                    CURRENT_STEP = Integer.parseInt(list.get(i).getStep());
                }
            }
        }
        if (mStepCount != null) {
...
//        filter.addAction(Intent.ACTION_USER_PRESENT);
        ...
        filter.addAction(MyBroadcastListener.ACTION_RUN_START);
        filter.addAction(MyBroadcastListener.ACTION_RUN_END);
        filter.addAction(MyBroadcastListener.ACTION_RUN_PARSE);
        filter.addAction(MyBroadcastListener.ACTION_RUN_RESUME);
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
                    ...
                    //时间变化步数重置为0 ,例如手动修改设置里的时间
                    isCall();
                    save();
                    isNewDay();
                } else if (Intent.ACTION_TIME_TICK.equals(action)) {
                    //日期变化步数重置为0,系统每分钟会发出该广播
                    isCall();
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
                    save();
                    isNewDay();
                } else if (action.equals(MyBroadcastListener.ACTION_RUN_START)) {
                    //收到开始跑步的广播
                    updateRunStatus(RUN_STATUS_START);
                } else if (action.equals(MyBroadcastListener.ACTION_RUN_END)) {
                    //收到结束跑步的广播
                    updateRunStatus(RUN_STATUS_END);
                } else if (action.equals(MyBroadcastListener.ACTION_RUN_PARSE)) {
                    //收到暂停跑步的广播
                    updateRunStatus(RUN_STATUS_PARSE);
                } else if (action.equals(MyBroadcastListener.ACTION_RUN_RESUME)) {
                    updateRunStatus(RUN_STATUS_RESUME);
                }
            }
        };
    ...
    private void isNewDay() {
        ...
        if (time.equals(new SimpleDateFormat("HH:mm").format(new Date()))
                || !CURRENT_DATE.equals(getTodayDate())) {
            initTodayData();
        }
    }
...
//            remindNotify();
    ...
    private void updateNotification() {
        Class cls = AllActivity.class;
        Intent hangIntent = new Intent(this, cls);
...
//            Logger.d("tempStep" + tempStep);
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (TextUtils.isEmpty(stepData.getSecond())) {
                    totalStepIndex = i;
                    //只修改总的步数
                    StepData data = list.get(totalStepIndex);
                    data.setStep(tempStep + "");
                    DbUtils.update(data);
                }
            }
    ...
    private int runStatus;//跑步的状态
    private final int RUN_STATUS_START = 1;//开始跑步
    private final int RUN_STATUS_END = 2;//结束跑步
    private final int RUN_STATUS_PARSE = 3;//暂停跑步
    private final int RUN_STATUS_RESUME = 4;//暂停后开始跑步

    private int startStep;//开始跑步步数
    private int endStep;//结束跑步步数
    private int totalStep;//总计跑步步数
    private int runningSteps;//已经跑的步数
    private long runningSeconds;//已经跑的时间

    private long totalTime;//总计跑步的时间，毫秒数
    private long runTimeStart;//开始跑步的毫秒数
    private long runTimeEnd;//结束跑步的毫秒数
    private boolean isRunning;//是否正在跑步
    ...
    private Thread thread = new Thread(new Runnable() {
        ...
        public void run() {
            while (isRunning) {
                ...
                setRunningDatas();
                MyBroadListenerIml.getInstance(StepService.this).onStepChanged(runningSteps, runningSeconds);
            }
        }
    });

    private void setRunningDatas() {
        if (totalStep == 0) {
            runningSteps = CURRENT_STEP - startStep;
        } else {
//            runningSteps = CURRENT_STEP - startStep + totalStep;
            runningSteps = CURRENT_STEP - startStep;
        }
        if (totalTime < 1) {
            runningSeconds = new Date(System.currentTimeMillis()).getTime()
                    - runTimeStart;
        } else {
            runningSeconds = new Date(System.currentTimeMillis()).getTime()
                    - runTimeStart + totalTime;
        }
    }

    private void updateRunStatus(int type) {
        runStatus = type;
        switch (type) {
            case RUN_STATUS_RESUME:
                if (!isRunning) {
                    thread.start();
                }
                isRunning = true;
                runTimeStart = new Date(System.currentTimeMillis()).getTime();
                break;
...
//                isRunning = false;
        ...
        Log.e("wade", "步数runningSteps：" + runningSteps + ",endStep:" + endStep + ",startStep:" + startStep + ",totalStep:" + totalStep);
    }

    private void getTotalRunData() {
        totalStep = endStep - startStep + totalStep;
        //计算从开始到暂停的毫秒数
        runTimeEnd = new Date(System.currentTimeMillis()).getTime();
        totalTime = runTimeEnd - runTimeStart + totalTime;
    }
    ...
    private void saveRun() {
        ...
        //记录跑步的数据
        if (runStatus == RUN_STATUS_END) {
            StepData dataRun = new StepData();
            dataRun.setToday(CURRENT_DATE);
            dataRun.setStep(totalStep + "");
            dataRun.setSecond(DateUtils.getSecondByMs(totalTime));
            DbUtils.insert(dataRun);
            initRunData();
        }
    }
